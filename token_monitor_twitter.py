...
class TwitterAnalyzer:
    ...
    async def analyze_token(self, contract_address: str) -> Dict:
        ...
        try:
            ...
            metrics['mention_count'] = len(tweets)
            ...
            for tweet in tweets:
                ...
                if self._is_notable_account(tweet.author.username):
    ...
    async def _find_official_account(self, contract_address: str) -> Optional[Dict]:
        ...
        try:
            ...
            accounts = self.client.search_recent_tweets(
                query=query,
                max_results=10,
                tweet_fields=['created_at'],
                user_fields=['username', 'verified', 'created_at', 'description'],
                expansions=['author_id']
            )
            ...
            # Find most likely official account based on verification and mention patterns
            # This could be enhanced with more sophisticated matching
            return {
                'id': accounts.data[0].author_id,
                'username': accounts.includes['users'][0].username
            }
    ...
    async def _track_name_changes(self, account_id: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for tweet in tweets.data:
                if tweet.author.username != current_name:
                    name_changes.append({
                        'timestamp': tweet.created_at.isoformat(),
                        'name': tweet.author.username
                    })
                    current_name = tweet.author.username
